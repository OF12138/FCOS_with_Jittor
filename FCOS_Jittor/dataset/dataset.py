...
def preprocess_img_boxes(image, boxes, input_size):
    ...
    # 1. Calculate scale to fit within max_side while maintaining aspect ratio
    scale = min(max_side / h, max_side / w)
    nh, nw = int(h * scale), int(w * scale)
    image_resized = cv2.resize(image, (nw, nh))

    # 2. Create a square padded image
    image_paded = np.full((max_side, max_side, 3), 128, dtype=np.uint8)
    # 3. Paste the resized image into the top-left corner
    image_paded[:nh, :nw, :] = image_resized

    # 4. Scale bounding boxes to match the resized image
    if boxes.shape[0] > 0:
        boxes[:, :4] *= scale

    # The `scale` value returned should be the one used for transformation
    return np.transpose(image_paded, (2, 0, 1)), boxes, scale
