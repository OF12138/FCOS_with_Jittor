...
class FCOS(nn.Module):
    ...
    def forward(self,x):
        ...
        '''
        #cls is classifacation and cnt is centerness and reg is regression
        #[batch_size,channels,height,width]
        #five means five different resolution
        C3,C4,C5=self.backbone(x)
        all_P=self.fpn([C3,C4,C5])
        cls_logits,cnt_logits,reg_preds=self.head(all_P)
        return [cls_logits,cnt_logits,reg_preds]

#post-processing, converting the raw result to filtered bounding box
#I think the class should be renamed to post-processing
class DetectHead(nn.Module):
    ...
    def __init__(self,score_threshold,nms_iou_threshold,max_detection_boxes_num,strides,config=None):
        ...
        self.nms_iou_threshold=nms_iou_threshold  #overlapping
    ...
    def forward(self,inputs):
        ...
        reg_preds   list contains five [batch_size,4,h,w] 
        '''
        #the fusion of different resolution*(I guess)
        ...
        cls_preds=cls_logits.sigmoid_() #get the result confidence probabilities between 0 to 1
        cnt_preds=cnt_logits.sigmoid_()

        coords =coords.cuda() if torch.cuda.is_available() else coords

        #Finds the maximum classification score for each location and its corresponding class index
        ##i don't understand
        ...
        #filtering the low-confidence boxes -Gemini
        #top-k selection
        ##I can't understand the algorithm
        ...
        #applies final filtering and Non-Maximum Suppression to the top-k predictions
    ...
    #the functions below are helper funcs
    ...
    #the helper function which turns the ltrb to real bounding boxes
...
#to prevent bounding box out of the boundries of image
class ClipBoxes(nn.Module):
    def __init__(self):
        super().__init__()
    def forward(self,batch_imgs,batch_boxes):
        batch_boxes=batch_boxes.clamp_(min=0)
        h,w=batch_imgs.shape[2:] #to get the height and width of the image
        batch_boxes[...,[0,2]]=batch_boxes[...,[0,2]].clamp_(max=w-1)
        batch_boxes[...,[1,3]]=batch_boxes[...,[1,3]].clamp_(max=h-1)
        return batch_boxes

#wrapper of the fcoe and the detection head
class FCOSDetector(nn.Module):
    ...
    def forward(self,inputs):
        ...
        if self.mode=="training":
            ...
            out=self.fcos_body(batch_imgs) #feature extracting

            scores,classes,boxes=self.detection_head(out) #post-processing

            boxes=self.clip_boxes(batch_imgs,boxes) #boundries caution
            return scores,classes,boxes
